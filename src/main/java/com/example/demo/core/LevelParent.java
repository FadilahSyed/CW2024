...
public abstract class LevelParent {
	...
	public LevelParent(String backgroundImageName, double screenHeight, double screenWidth, int playerInitialHealth) {
		...
		this.scene=new Scene(root,screenWidth,screenHeight);
		this.user=new UserPlane(playerInitialHealth);
		this.background=initializeBackground(backgroundImageName);
	...
	private ImageView initializeBackground(String imageName) {
		ImageView bg = new ImageView(ImageLoader.load(imageName));
		bg.setFitHeight(screenHeight);
		bg.setFitWidth(screenWidth);
		bg.setOnKeyPressed(this::handleKeyPress);
		bg.setOnKeyReleased(this::handleKeyRelease);
		return bg;
	}
	...
	private void handleCollisions(List<ActiveActorDestructible> group1, List<ActiveActorDestructible> group2) {
		for (ActiveActorDestructible a1 : group1) {
			for (ActiveActorDestructible a2 : group2) {
	...
	private void removeDestroyedActorsFromList(List<ActiveActorDestructible> actors) {
		List<ActiveActorDestructible> toRemove = actors.stream().filter(ActiveActorDestructible::isDestroyed).collect(Collectors.toList());
